...
# Read state and district shapefiles of India from github
...
#State shape file
ind_state_shape <- st_read("Shape_files/state.shp")

#District shape file
ind_district_shape <- st_read("Shape_files/district.shp")

# #Read state and district shapefiles of India from computer
# ind_state_shape <- st_read("C:/Shape files 2020/state.shp")
# ind_district_shape <- st_read("C:/Shape files 2020/district.shp")

# Define UI for app
ui <- fluidPage(titlePanel("Vaccination Coverage in India"),
                tabsetPanel(
                  tabPanel("State-wise Map",
                           sidebarLayout(
                             ...
                             mainPanel(plotOutput("ind_map_state"))
                           )),
                  tabPanel("District-wise Map",
                           sidebarLayout(
                             ...
                             mainPanel(plotOutput("ind_map_district"))
                           ))
                ))
...
server <- function(input, output) {
  ...
  ind_map_data_state <-
    merge(ind_state_shape, ind_data, by.x = "OBJECTID", by.y = "OBJECTID")
  ind_map_data_district <-
    merge(ind_district_shape,
          ind_data,
          by.x = "OBJECTID",
          by.y = "OBJECTID")
  ...
  output$ind_map_state <- renderPlot({
    ggplot(ind_map_data_state, aes(fill = !!sym(input$var_state))) +
      ...
      labs(
        title = "Coverage of Vaccination in India - State-wise",
  ...
  output$ind_map_district <- renderPlot({
    ggplot(ind_map_data_district, aes(fill = !!sym(input$var_district))) +
      ...
      scale_fill_viridis_b() +
      labs(
        title = "Coverage of Vaccination in India - District-wise",
